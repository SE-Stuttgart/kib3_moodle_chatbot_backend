...
def create_json_from_template(domain: JSONLookupDomain, template_filename: str, language: str = "English"):
    template = RegexFile(template_filename, domain)
    domain_name = domain.get_domain_name()
    _write_dict_to_file(_create_request_json(domain, template), f'{domain_name}{"" if language == "English" else language}RequestRules.json')
    _write_dict_to_file(_create_inform_json(domain, template), f'{domain_name}{"" if language == "English" else language}InformRules.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument("language", help="English or German", choices=["English", "German"], default="English")
    args = parser.parse_args()
    nlu_file = os.path.join(head_location, 'resources', 'nlu_regexes', f"{args.filename}.nlu")
    dom = JSONLookupDomain(args.domain)
    create_json_from_template(dom, nlu_file, args.language)
