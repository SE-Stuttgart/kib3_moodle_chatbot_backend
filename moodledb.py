...
def get_badges(sesssion, user_id: int):
	badges = session.query(MBadge).all()
	for badge in badges:
		# replace placeholder badgename by name of badge
		# TODO where to get %badgelink% from? 
		badge.message = badge.message.replace('%badgename%', badge.name)
	return badges

def get_completed_course_modules(session, user_id: int):
	return session.query(MCourseModulesCompletion).filter(MCourseModulesCompletion.userid==user_id, MCourseModulesCompletion.completionstate==1)

def get_not_completed_course_modules(session, user_id: int):
	return session.query(MCourseModulesCompletion).filter(MCourseModulesCompletion.userid==user_id, MCourseModulesCompletion.completionstate==0)

def get_courses(session):
	return session.query(MCourse).all()

def get_course_sections(session, course_id: int):
	return session.query(MCourseSection).filter(MCourseSection.course==course_id)

def get_course_modules(sesssion, course_id: int, section: int):
	return session.query(MCourseModule).filter(MCourseModule.course==course_id, MCourseModule.section==section)

def get_user_by_name(session, user_name: str):
	return session.query(MUser).filter(MUser.username=="student1")[0]

class GradeInfo:
	# Helper Object to accumulate grade info across multiple tables (m_grade_items, m_grade_grades)
	def __init__(self, course_id: int, item_name: str, item_type: str, item_module: str, user_id: int, rawgrade_min: float, rawgrade_max: float, rawgrade: float, finalgrade: float):
		self.course_id = course_id
		self.item_name = item_name
		self.item_type = item_type
		self.item_module = item_module
		self.user_id = user_id
		self.rawgrade_min = rawgrade_min
		self.rawgrade_max = rawgrade_max
		self.rawgrade = rawgrade
		self.finalgrade = finalgrade
		self.passed = rawgrade >= rawgrade_min if rawgrade != None and rawgrade_min != None else None
	
	def __repr__(self) -> str:
		return f"{self.item_name} ({self.item_type, self.item_module}): {self.rawgrade}/{self.rawgrade_max} Punkte, mindestens benoetigt: {self.rawgrade_min}. Final: {self.finalgrade} (bestanden: {self.passed})"

def get_grades(session, user_id: int, course_id: int) -> List[GradeInfo]:
	grade_items = session.query(MGradeItem).filter(MGradeItem.courseid==course_id) # get only grade items relevant to course
	gradeinfo = []
	for grade_item in grade_items:
		grades = session.query(MGradeGrade).filter(MGradeGrade.itemid==grade_item.id, MGradeGrade.userid==user_id)
		for grade in grades:
			gradeinfo.append(GradeInfo(course_id=course_id, item_name=grade_item.itemname, item_type=grade_item.itemtype, item_module=grade_item.itemmodule,
...
def print_users(session):
	print("--------- Users ---------")
	for user in session.query(MUser).all():
		print("User", user.username)
	print("-------------------------")

def print_courses(session):
	print("--------- Courses ---------")
	for course in get_courses(session):
		print("Course", course.shortname)
		for sec in get_course_sections(session, course_id=course.id):
...
with Session() as session:
	print_users(session)
	print_courses(session)

	print("\n\n\n")
	ml_course = get_courses(session)[1]
	print("Selected Course", ml_course.shortname)

	user = get_user_by_name(session, "student1")
	print("Selected User", user.username, "id:", user.id)
	user_id = 3

	# print 
	print("Not yet completed modules")
	for not_compl in get_not_completed_course_modules(session, user.id):
		print(not_compl)

	print("Grades in course", ml_course.shortname)
	for grade in get_grades(session, user_id, ml_course.id):
